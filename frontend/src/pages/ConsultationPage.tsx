...
import { createDiagnosticOrder, createPrescription, generateCaseStudy, getAppointment, getMedicines, updateAppointmentNotes, updateAppointmentStatus } from '../api/client';
import { SCAN_TYPES } from '../types';
import type { Appointment, AppointmentStatus, Medication, Medicine, ScanType } from '../types';
import MedicineSearchSelect from '../components/MedicineSearchSelect';

const emptyMedication: Medication = { medicine_id: null, name: '', dosage: '', frequency: '', duration: '' };
...
export default function ConsultationPage() {
  ...
  useEffect(() => { getMedicines().then(setMedicineCatalog).catch(() => setError('Could not load the medicine catalog.')); }, []);
    ...
    <div className="workspace-page portal-page">
      ...
      <div className="clinical-layout">
        ...
        <main className="consultation-stack">
          ...
          <section className="portal-card"><header><div><p className="eyebrow">Treatment</p><h2>Prescription and follow-up</h2></div><span className="status-pill">Searchable medicine catalog</span></header><div className="form-grid"><label className="field field--wide"><span>Working diagnosis</span><input value={diagnosis} onChange={(event) => setDiagnosis(event.target.value)} /></label>{medications.map((medication, index) => <div className="medication-row" key={index}><label className="field medicine-select-field"><span>Medicine name</span><MedicineSearchSelect medicines={medicineCatalog} value={medication.medicine_id} ariaLabel={`Medicine ${index + 1}`} onChange={(selected) => setMedications(medications.map((item, itemIndex) => itemIndex === index ? { ...item, medicine_id: selected?.id || null, name: selected?.name || '' } : item))} /></label>{(['dosage', 'frequency', 'duration'] as const).map((field) => <label className="field" key={field}><span>{field}</span><input value={medication[field]} onChange={(event) => setMedications(medications.map((item, itemIndex) => itemIndex === index ? { ...item, [field]: event.target.value } : item))} placeholder={field === 'frequency' ? 'e.g. twice daily or 2 doses/day' : field === 'duration' ? 'e.g. 1 month' : ''} /></label>)}</div>)}<button className="text-button" type="button" onClick={() => setMedications([...medications, { ...emptyMedication }])}>＋ Add medication</button><label className="field field--wide"><span>Instructions / treatment plan</span><textarea rows={3} value={instructions} onChange={(event) => setInstructions(event.target.value)} /></label><label className="field field--wide"><span>Follow-up plan</span><textarea rows={2} value={followUp} onChange={(event) => setFollowUp(event.target.value)} /></label></div><div className="flow-actions"><button className="button" disabled={!medications.some((item) => item.medicine_id)} onClick={() => safe(prescribe)}>Save prescription</button><button className="button button--primary" onClick={() => safe(createCase)}>Generate case study</button></div></section>
